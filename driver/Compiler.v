...
Definition CompCert's_passes_from_Cminor :=
    mkpass Selectionproof.match_prog
::: mkpass RTLgenproof.match_prog
::: mkpass (match_if Compopts.optim_tailcalls Tailcallproof.match_prog)
::: mkpass Inliningproof.match_prog
...
Definition match_prog_cminor: Cminor.program -> Asm.program -> Prop :=
  pass_match (compose_passes CompCert's_passes_from_Cminor).

Theorem transf_cminor_program_match:
  forall p tp,
  transf_cminor_program p = OK tp ->
  match_prog_cminor p tp.
Proof.
  intros p4 tp T.
  unfold transf_cminor_program, time in T. rewrite ! compose_print_identity in T. simpl in T.
  destruct (Selection.sel_program p4) as [p5|e] eqn:P5; simpl in T; try discriminate.
...
Proof.
  ...
  split. auto.
  apply forward_to_backward_simulation. assumption.
  apply Cminor.semantics_receptive. apply Asm.semantics_determinate.
Qed.
...
Proof.
  intros p tp T.
  eapply (proj2 (cminor_semantic_preservation p tp _)). Unshelve.
  apply transf_cminor_program_match; auto.
Qed.
...
Proof.
  ...
  apply transf_cminor_program_correct; eassumption.
Qed.