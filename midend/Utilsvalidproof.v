...
Definition erase_instr instr : RTL.instruction :=
  match instr with 
    ...
    | Ibuiltin fn args dst succ =>
      RTL.Ibuiltin fn
                   (List.map (map_builtin_arg (fun a => (erase_reg a))) args)
...
Proof. 
 ...
 - (* builtin *)
   ...
   replace (map erase_reg (params_of_builtin_arg a))
     with (params_of_builtin_arg (map_builtin_arg (λ a0 : reg, erase_reg a0) a)); auto.
   clear IHl.
...
| wt_Ibuiltin_res: forall γ ef args s r i,
  ...
  γ ⊢ (Ibuiltin ef args (BR (r,i)) s) ; γ[r←i]
