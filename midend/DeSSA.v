...
Proof.
  ...
  congruence.
  intros.
  generalize (H4 pc) (H7 pc). intuition auto.
  left ; congruence.
  right ; congruence.
Qed.
    ...
    let nx_cp := st_nextnode_cp s in 
      let nxt := (next code (Psucc nx_cp) (((nat_of_P max) - (S (nat_of_P pc)))%nat)) in
        match plt nxt (st_first_fs s) with 
          ...
          | right _ => Error (Errors.msg "DeSSA 1")
        end.
  ...
  match ros with 
    | inl r => 
      if (Bij.valid_index size (snd r)) then Errors.OK (inl ident (Bij.pamr size r))
        else Errors.Error (Errors.msg "DeSSA 4")
  ...
  match ins with 
    ...
    | Itailcall sig ros args => 
      if (forallb (valid_ind size) args)
        ...
        else Errors.Error (Errors.msg "DeSSA 11")
  ...
  fun s =>
    match code ! pc with 
      | None => 
        match plt (Psucc (st_nextnode_cp s)) (st_first_fs s) with 
          ...
          | right _ => Error (Errors.msg "DeSSA 17") 
        end
      | Some (Inop succ) => 
        match pcode ! succ with 
          ...
          | Some block => 
            match index_pred preds pc succ with
              | None => Error (Errors.msg "DeSSA 18") 
    ...
    | Some ins => 
      match (map_pamr size ins) with 
        | Errors.Error _ => Error (Errors.msg "DeSSA 19")
...
Module PosOrder <: TotalLeBool.
  ...
  Theorem leb_total : forall a1 a2, a1 <=? a2 = true \/ a2 <=? a1 = true.
    Proof. 
      intros.
      unfold leb.
      destruct (Ple_total a1 a2); (destruct (Ple_dec); intuition auto).
      right. destruct (Ple_dec); intuition auto.
    Qed.
End PosOrder.
...
Definition transl_function (f: SSA.function) : Errors.res RTL.function :=
          ...
          | OK u s'' H => 
            if (forallb (valid_ind (fn_max_indice f)) f.(SSA.fn_params)) then
              Errors.OK (RTL.mkfunction
                ...
                s''.(st_code)
                f.(SSA.fn_entrypoint))
              else Errors.Error (Errors.msg "DeSSA 20")
        end
        else Errors.Error (Errors.msg "DeSSA 21").
