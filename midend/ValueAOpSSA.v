...
Require Import Op SSA  ValueDomain ValueDomainSSA.
...
Definition eval_static_addressing_32 (addr: addressing) (vl: list aval): aval :=
  ...
  | Aglobal _ _, nil => Vtop
  | Abased _ _, _::nil => Vtop
  | Abasedscaled _ _ _, _::nil => Vtop
  | Ainstack _, nil => Vtop
  | _, _ => Vbot
  end.
...
Definition eval_static_operation (op: operation) (vl: list aval): aval :=
  ...
  | Ofloatconst n, nil => if propagate_float_constants tt then F n else Vtop
  | Osingleconst n, nil => if propagate_float_constants tt then FS n else Vtop
  | Oindirectsymbol id, nil => Vtop
...
Theorem eval_static_addressing_32_sound:
  forall (ge: genv) sp addr vargs vres aargs,
  eval_addressing32 ge sp addr vargs = Some vres ->
  list_forall2 vmatch vargs aargs ->
  vmatch vres (eval_static_addressing_32 addr aargs).
Proof.
...
Theorem eval_static_addressing_64_sound:
  forall (ge: genv) sp addr vargs vres aargs,
  eval_addressing64 ge sp addr vargs = Some vres ->
  list_forall2 vmatch vargs aargs ->
  vmatch vres (eval_static_addressing_64 addr aargs).
Proof.
...
Theorem eval_static_addressing_sound:
  forall (ge: genv) sp addr vargs vres aargs,
  eval_addressing ge sp addr vargs = Some vres ->
  list_forall2 vmatch vargs aargs ->
  vmatch vres (eval_static_addressing addr aargs).
Proof.
...
Theorem eval_static_operation_sound:
  forall (ge: genv) sp op vargs m vres aargs,
  eval_operation ge sp op vargs m = Some vres ->
  list_forall2 vmatch vargs aargs ->
