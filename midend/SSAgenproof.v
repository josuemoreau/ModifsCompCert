...
Lemma map_partial_first : forall lp ltp,
  transf_globdefs (fun _ => transf_fundef) (fun _ (v:unit)  => OK v) lp = OK ltp ->
  exists ox,
    transf_globdefs (fun _ => RTLnorm.transl_fundef_opt) (fun _ v => OK v) lp = OK ox.
Proof.
  induction lp; intros.
  - simpl in *.
    inv H. eauto.
...
Lemma map_partial_second : forall lp tp ox,
  transf_globdefs (fun _ => transf_fundef) (fun _ (v : unit) => OK v) lp = OK tp ->
  transf_globdefs (fun _ => RTLnorm.transl_fundef_opt) (fun _ (v : unit) => OK v) lp = OK ox ->
  exists ox',
    transf_globdefs (fun _ => RTLdfs.transf_fundef) (fun _ (v : unit) => OK v) ox = OK ox'.
Proof.
  induction lp; intros.
  - simpl in *.
...
Lemma map_partial_trans:
  forall lp ltp ox ox',
    ...
    transf_globdefs (fun _ => RTLnorm.transl_fundef_opt) (fun _ (v : unit) => OK v) lp = OK ox ->
    transf_globdefs (fun _ => RTLdfs.transf_fundef) (fun _ (v : unit) => OK v) ox = OK ox' ->
    transf_globdefs (fun _ => SSAvalid.transf_fundef) (fun _ (v : unit) => OK v) ox' = OK ltp.
Proof.
  induction lp ; intros.
...
Proof.
  ...
  eexists {| prog_defs := ox; prog_public:= prog_public p ; prog_main := prog_main p |}.  
  exists {| prog_defs := ox'; prog_public:= prog_public p ; prog_main := prog_main p |}.  
...
Section PRESERVATION.
  ...
  Definition match_prog (p: RTL.program) (tp: SSA.program) :=
    match_program (fun cu f tf => SSAgen.transf_fundef f = Errors.OK tf) eq p tp.

  Lemma transf_program_match:
    forall p tp, SSAgen.transf_program p = OK tp -> match_prog p tp.
  Proof.
...
Proof.
  ...
  - unfold transf_fundef in Htrans.
    ...
    + monadInv EQ. 
      ...
      * unfold RTLdfs.transf_fundef.
        simpl ; unfold bind ; rewrite EQ ; eauto.
      * exploit RTLdfsproof.transf_function_wf_dfs; eauto. intros [dfsl [Hdfs CONC]].
        econstructor; eauto.
...
Lemma virt_match_tp : forall p tp, 
  match_prog p tp -> 
  exists tp_norm, exists tp_dfs, 
    RTLnormproof.match_prog p tp_norm 
    /\ RTLdfsproof.match_prog tp_norm tp_dfs 
    /\ SSAvalidproof.match_prog tp_dfs tp.
Proof.
  intros p tp MATCH.
  Print RTLnorm.transl_program.
