...
Proof.
  ...
  sz.
  inv H2; simpl in *. intuition auto.  
  sz.
...
Proof.
  ...
  exploit empty_block_at ; eauto. intros. intuition auto.
  inv H4.
...
Proof.
  ...
  destruct ins;  
    ...
    try solve [((econstructor 3 ; eauto ; try kick ; try congruence);
      (exploit copy_ins_at ; eauto ; (intros ; intuition auto ; (inv H2 ; eauto) )))]).

  Focus 2.  (* call *)
  econstructor 3 ; eauto; try congruence.
  econstructor 6 ; eauto.
...
Proof.
  ...
  destruct (plt maxacc a); eapply IHl ; eauto;
    ((eapply NoDup_list_norepet in H3);
      (eapply Coqlib.list_norepet_app in H3; intuition auto);
      (eapply list_norepet_NoDup ; eauto);
      (eapply Coqlib.list_norepet_app; intuition auto ; auto);
...
Section PRESERVATION.
  ...
  Definition match_prog (p: SSA.program) (tp: RTL.program) :=
    match_program (fun _ f tf => transl_fundef f = Errors.OK tf) eq p tp.
...
(* Qed. *)
    ...
    Proof.
      ...
      intros r1 Hnth.  
      simpl; rewrite Hnth. 
      assert (r1 <> r) by (intro H; inv H; exploit (RNOTUSED r0 l); auto;
                           try (constructor; auto); eapply nth_error_some_in; eauto). 
      ...
      eapply IHblock ; eauto;
        [ unfold para_block; intros;  intro;
          assert (Hcont := PRMNOTUSED dst0 args);
          assert (Hcont' := Hcont arg); eauto; clear Hcont;
          exploit Hcont' ; eauto
        | inv NODUP; auto].
    ...
    Lemma phi_store_e_copy:
      forall size k block rs dst arg args
             (PRMNOTUSED: para_block block)
             (NODUP: NoDup block)
             (NODUP': (forall r args args', 
                          In (Iphi args r) block -> In (Iphi args' r) block -> args = args'))
    ...
    Lemma reach_moves_star :  forall size ts phib sp rs  m tf  succ' lnew k lastnew,
        ...
        star RTL.step tge (RTL.State ts tf sp succ' rs m) E0
             (RTL.State ts tf sp lastnew (phi_store_e size k phib rs) m).
    Proof.
      induction phib; intros.
      simpl in *.  inv H.
    ...
    Proof.    
      ...
      - (* inop with block *)  
        ...
        exists  (RTL.State ts tf sp succ (phi_store_e (fn_max_indice f) k block rs') m). split.
        eapply plus_left with (s2 := (RTL.State ts tf sp succ' rs' m)) (t1:=E0) (t2:=E0) ; eauto.
        econstructor ; eauto.
        
        eapply star_right with (s2 := (RTL.State ts tf sp lastnew (phi_store_e (fn_max_indice f) k block rs') m)) (t1:= E0) (t2:= E0) ; eauto.
        ...
        intros.  generalize (H6 args r H2). intros Hvalid. unfold is_valid_phi in Hvalid.
        exploit Hvalid ; eauto.
        (* po1 *)  inv H3. eauto. 
      ...
      - (* ibuiltin *)
        ...
        + exists  (RTL.State ts tf sp pc'
                           (regmap_setres (map_builtin_res (Bij.pamr (fn_max_indice f)) res) vres rs') m'). split.
          * { eapply plus_one ; eauto. 
              eapply RTL.exec_Ibuiltin with (vargs := vargs) ; eauto.
              * inv H4.
                revert TRANSF_PROG H0 MREG H6. clear.
                { induction 2 ; simpl ; intros.
                  - constructor.
                  - constructor.
                    + assert (ARGSOK: forall r, In r (params_of_builtin_arg a1) ->
                                                Bij.valid_index (fn_max_indice f) (snd r) = true).
                      { intros; apply H6.
                        apply in_or_app; right.
                        apply in_or_app; left; auto.
                      }
                      eapply eval_builtin_arg_preserved with (1:= symbols_preserved) ; auto.
                      revert H ARGSOK MREG. clear.
                      { induction 1 ; simpl; intros; try econstructor; eauto.
                        - inv MREG.
                          erewrite H ; eauto.
                          econstructor.
                        - inv MREG.
                          eapply IHeval_builtin_arg1; eauto; try constructor; auto.
                          intros ; eapply ARGSOK ; eapply in_or_app; eauto.
                        - inv MREG.
                          eapply IHeval_builtin_arg2; eauto; try constructor; auto.
                          intros ; eapply ARGSOK ; eapply in_or_app; eauto.
                        - inv MREG.
                          eapply IHeval_builtin_arg1; eauto; try constructor; auto.
                          intros ; eapply ARGSOK ; eapply in_or_app; eauto.
          ...
          * { kick_valid.
            ...
            constructor. intros.
            destruct res.
              - destruct (p2eq x r); simpl.
                * inv e.
                  rewrite P2Map.gss. rewrite PMap.gss; auto.
                * rewrite P2Map.gso; auto. rewrite PMap.gso; auto. inv MREG ; auto.
                  intro Hcont. destruct r ; destruct x; simpl in *.
                  eapply Bij.INJ' in Hcont ; eauto.
                  exploit H6; eauto.
              - simpl. inv MREG; auto.
              - simpl. inv MREG; auto.
