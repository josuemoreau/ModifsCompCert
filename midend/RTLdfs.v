...
Definition dfs (tf: RTL.function) : res (list node * RTLt.code) := 
  do (res, code') <-
    (acc_succ 
      (RTL.fn_code tf)
...
Definition transf_function (f: RTL.function) : res RTLt.function :=
  do (seen,code) <- dfs f ;
    OK (RTLt.mkfunction
      (RTL.fn_sig f)
      (RTL.fn_params f)
...
Definition transf_fundef (fd: RTL.fundef) : res RTLt.fundef :=
  AST.transf_partial_fundef transf_function fd.

Definition transf_program (p: RTL.program) : res RTLt.program :=
...
Record wf_dfs_function (f: RTLt.function) : Prop := {
  fn_dfs_comp : forall j ins, (RTLt.fn_code f) ! j = Some ins -> In j (fn_dfs f);
  fn_dfs_reached: forall j, In j (fn_dfs f) -> (cfg (RTLt.fn_code f))** (RTLt.fn_entrypoint f) j;
