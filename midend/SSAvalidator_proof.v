      ...
      (fun phis x xdef =>
        if Regset.mem x live then
          let phi_args := List.map
            (fun pc0 => (x,read_gamma (get_Gpreds pc0) x)) pred in
...
Lemma in_params_of_builtin_args_erase : forall g l r i,
    In (r, i) (params_of_builtin_args (map (map_builtin_arg (λ x : Registers.reg, (x, read_gamma g x))) l)) ->
    read_gamma g r = i.
Proof.
...
Lemma map_builtin_res_id : forall i br,
    map_builtin_res erase_reg (map_builtin_res (fun r => (r, i)) br) = br.
Proof.
  clear.
...
Proof.
  ...
  - intros G new_code juncpoints G' new_code' juncpoints'.
      ...
      * { case_eq (def_phi ! n); [intros dphi Hdphi|simpl; intros; kill_error].
          ...
          - reduce_forall3_ptree.
            ...
            elim IHppoints with (1:=Heq) (2:=Hln2);
              [intros HINV [HINV1 [HINV4 [HINV2 [HINV5 [HINV3 [HINV6 [HINV7 [HINV8 [HINV9 [HINV10 [HINV11 HINV12]]]]]]]]]]]
              |eauto|eauto|idtac|eauto].
            + and_proof; auto.
    ...
    + { (* Iop *)
        ...
        elim IHppoints with (1:=Heq) (2:=Hln2);
          [intros HINV [HINV1 [HINV4 [HINV2 [HINV5 [HINV3 [HINV6 [HINV7 [HINV8 [HINV9 [HINV10 [HINV11 HINV12]]]]]]]]]]]
          |idtac|idtac|idtac|idtac]; clear Heq.
        - and_proof; auto.
          + intros; eapply HINV; eauto.
            rewrite PTree.gsspec; destruct peq; auto.
            subst.
    ...
    + { (* Ibuiltin *)
        ...
        - and_proof; auto.
          ...
          + intros i Hin; destruct Hin; [subst|apply HINV11; auto; fail].
            rewrite (HINV1 _ _ (PTree.gss _ _ _)); rewrite Hins; auto.
            unfold erase_instr, get_opt ; simpl.
            assert ((map (map_builtin_arg erase_reg) (map (map_builtin_arg (λ x0, (x0, read_gamma g x0))) l)) = l).
            { generalize l. clear.
              induction l ; intros; auto.
              simpl. rewrite IHl; auto.
              clear IHl.
              replace (map_builtin_arg erase_reg (map_builtin_arg (λ x0 : Registers.reg, (x0, read_gamma g x0)) a)) with a; auto.
              clear l. 
              induction a ; intros; auto; simpl;
        ...
        - and_proof; auto.
          ...
          + intros i Hin; destruct Hin; [subst|apply HINV11; auto; fail].
            ...
            rewrite H at 1; auto.
            rewrite ! map_builtin_res_id; auto.
...
Theorem typecheck_function_correct : forall f size def def_phi live f_ssa,
  ...
  exists G, wt_function f f_ssa live G /\ wf_init f_ssa G /\ check_erased_spec f f_ssa /\ normalised_function f 
   ...
   /\ (forall i, get_opt erase_instr ((f_ssa.(fn_code))!i) = get_opt (fun x => x) ((f.(RTLt.fn_code))!i))
.
Proof.
  unfold typecheck_function; intros f size def def_phi live f_ssa HOK HDFS HNOREPET.  
  ...
  - intros [H [H0 [H1 [H2 [H3 [H4 [H5 [H6 [H7 [H8 [H9 [H10 H11]]]]]]]]]]]].
                ...
                { econstructor 2; eauto.
                  ...
                  - constructor; simpl.
                    ...
                    set (ff:={|
                               ...
                               fn_ext_params := ext_params_list new_code phiblock (map (λ r : Registers.reg, (r, dft_pos)) (RTLt.fn_params f));
                               fn_dom_test := domtest
                             |}).
                    exploit fold_build_phi_block_def_phi_some; eauto; intros [d [Hd _]]; eauto.
                    unfold is_joinpoint in *.
    ...
    + (** wf_init **)
      ...
      assert (TT: ∀pc : RTL.node, RTLutils.join_point pc f ↔ join_point pc {|
                             ...
                             (map (λ r : Registers.reg, (r, dft_pos)) (RTLt.fn_params f));
                                                        fn_dom_test := domtest |}).
      { intros pc. 
...
Proof.
  ...
  - destruct (fold_left
                (update_ctx  (make_predecessors (RTLt.fn_code f) RTLt.successors_instr)
                             def def_phi (RTLt.fn_code f) live) (fn_dfs f)
                (OK (entry_Gamma f, PTree.empty instruction, nil))); try congruence.
    destruct p as [[G newcode] jp].
    monadInv H.
    destruct (compute_test_dom (RTLt.fn_entrypoint f) newcode) ; try congruence.
...
Proof.
  ...
  destruct check_function_inv.
  destruct (fold_left
          (update_ctx (make_predecessors (RTLt.fn_code f) RTLt.successors_instr) def
  ...
  case_eq (check_unique_def {|
             ...
             fn_ext_params := ext_params_list t0 x
                                (map (λ r : Registers.reg, (r, dft_pos))
                                   (RTLt.fn_params f));
...
Proof.
  intros.
  assert (∀i : positive, get_opt erase_instr (fn_code tf) ! i = get_opt (fun x => x) (RTLt.fn_code f) ! i).
    exploit typecheck_function_correct ; eauto; [ eapply RTLdfs.fn_dfs_comp ; eauto|eapply RTLdfs.fn_dfs_norep ; eauto|idtac].
    intros [G HG].
  ...
  assert (forall i j, RTLutils.cfg (RTLt.fn_code f) i j -> cfg tf i j).
    ...
    inv H6.
    replace (RTLt.successors_instr (erase_instr i0)) with (successors_instr i0) in *.
    econstructor; eauto.
...
Proof.
  ...
  assert ((RTLt.fn_code f) ! pc = Some (erase_instr i0)).
  rewrite HCODE. unfold erase_code ; rewrite PTree.gmap ; unfold option_map.
  rewrite H6 ; auto.
...
Proof.
  ...
  case_eq (check_function_inv f
             (make_predecessors (RTLt.fn_code f) RTLt.successors_instr)) ; intros Hif ; rewrite Hif in *.
  clear H1. intros.
  intro Hcont. inv Hcont.
  inv Herased.
  assert ((RTLt.fn_code f) ! pc = (Some (erase_instr ins))).
  rewrite HCODE.
